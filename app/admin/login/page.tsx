  ...
  const loginWithFace = async () => {
    ...
    try {
      ...
      if (!window.PublicKeyCredential || !navigator.credentials) {
        throw new Error("This browser does not support passkey/biometric login. Try a recent Chrome or Edge browser.");
      }

      const { error } = await supabase.auth.signInWithPasskey();
      if (error) throw error;

      // Give the SSR client a moment to persist the authenticated session cookie
      // before navigating to the middleware-protected admin page.
...
export default function LoginPage() {
  return <Suspense fallback={<main className="flex min-h-screen items-center justify-center bg-slate-950 text-white">Loading login...</main>}><LoginForm /></Suspense>;
}
